...
func TestNewEdgeSyncerYAMLWithFeatureGates(t *testing.T) {
	expectedYAML := `---
apiVersion: v1
kind: Namespace
metadata:
...
func TestGetGroupMappingsForEdge(t *testing.T) {
	testCases := []struct {
		name     string
		input    []string
		expected []groupMappingForEdge
	}{
		{
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			actual := getGroupMappingsForEdge(tc.input)
			if len(tc.input) == 0 {
